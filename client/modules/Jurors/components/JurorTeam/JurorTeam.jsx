...
function JurorTeam() {
  return (
    <section className={styles.jurorTeam}>
      ...
      {JURORS.map(({ sectionTitle, groupMembers }) => (
        <div key={sectionTitle} className={styles.memberBox}>
          <h2>{sectionTitle}</h2>
          {groupMembers.map(({ memberName, memberTitle, memberImg, memberLink }) => (
            <div key={memberName} className={styles.memberInlineWrapper}>
